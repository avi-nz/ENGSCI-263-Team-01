...
stores_sat = durations_df_sat.index.tolist()
routes_index = [str(i) for i in range(num_routes)]

#ok building the matrix, wish me luck
A_sat = [[0 for j in range(num_routes)] for i in range(len(stores_sat))]
...
for i in range(len(routes_sat)):
  for j in range(len(routes_sat[i])):
    lease_route_cost[k] += 1400*math.ceil(routes_sat[i][j]["total_time_sec"]/7200)
    k+=1

exclusion_cost = [0 for i in range(len(stores_sat))]
for i in range(len(stores_sat)):
  if stores_sat[i][0] == "P":
...
prob = LpProblem("Saturdays", LpMinimize)
...
#LP FOR SATURDAYS with fuel reduction

vars_x = LpVariable.dicts("Route", routes_index, 0, None, LpBinary)
vars_y = LpVariable.dicts("Route_Lease", routes_index, 0, None, LpBinary)
vars_z = LpVariable.dicts("Skipped_Store", stores_sat, 0, None, LpBinary)
prob_fr = LpProblem("Saturdays_Fuel_Reduction", LpMinimize)
prob_fr += lpSum([vars_x[routes_index[i]]*route_cost[i] +
               vars_y[routes_index[i]]*lease_route_cost[i]
               for i in range(len(routes_index))] +
                 [vars_z[stores_sat[j]]*exclusion_cost[j] for j in range(len(stores_sat))]), "Cost of Routes and Skipping"
...
with open("stored_routes/chosen_routes_sat_fr.txt", "w") as file:
  json.dump(chosen_routes, file)
with open("stored_routes/leased_routes_sat_fr.txt", "w") as file:
  json.dump(leased_routes, file) #should be empty
with open("stored_routes/skipped_stores_sat_fr.txt", "w") as file:
